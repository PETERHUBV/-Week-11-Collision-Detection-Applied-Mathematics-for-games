...
public class LineGen2 : MonoBehaviour
{
    ...
    //  Customizable Colors 
    public Color inAirColor = Color.red;      // Color while in air
    public Color collisionColor = Color.green; // Color on platform
    private Color currentShapeColor;

    void Start()
    {
        currentShapeColor = inAirColor;
    }
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space) && onPlatform)
        {
            ...
            onPlatform = false;
            currentShapeColor = inAirColor; // Reset color while in air
        }
        ...
        if (shapePos.y - shapeSize / 2f <= platformTop)
        {
            shapePos.y = platformTop + shapeSize / 2f;
            velocityY = 0f;
            onPlatform = true;
            currentShapeColor = collisionColor; // Change color on collision
        }
    }

    void OnPostRender()
    {
        if (shapeMaterial == null || platformMaterial == null) return;
